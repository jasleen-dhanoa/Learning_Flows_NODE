...
def create_fig(exp, gyre_type, model_type, cbar):

    if exp == "test":
        # separate plotting for NODE & KNODE/ANODE
        if cbar:
...
def visualize_true_double_gyre(t1, true_y1, t2, true_y2, device, exp, model_type, flow_type):
    ...
    dydt = dynamics.forward(0, grid_samples).cpu().detach().numpy()
    dydt = dydt.reshape(1000, 1000, 2)
    ax_vecfield.scatter(true_y1.cpu().numpy()[0, :, 0], true_y1.cpu().numpy()[0, :, 1], c = 'blue', marker='o')
    ax_vecfield.scatter(true_y2.cpu().numpy()[0, :, 0], true_y2.cpu().numpy()[0, :, 1], c = 'green', marker='o')
    ax_vecfield.plot(true_y1.cpu().numpy()[:, :, 0], true_y1.cpu().numpy()[:, :, 1], 'b', label='true1')
    ax_vecfield.plot(true_y2.cpu().numpy()[:, :, 0], true_y2.cpu().numpy()[:, :, 1], 'g', label='true2')
    ax_vecfield.streamplot(x, y, dydt[:, :, 0], dydt[:, :, 1], color="black")
    ax_vecfield.set_xlim(-50, 50)
    ax_vecfield.set_ylim(-25, 75)
    ax_vecfield.set_title('True Vector Field')
    ax_vecfield.set_xlabel('x')
    ax_vecfield.set_ylabel('y')
    if exp == 'test':
...
def visualize_double_gyre_streamplot(itr, t1, t2, true_y1, true_y2, pred_y1, pred_y2, odefunc, fig, ax_traj, ax_traj2, ax_vecfield, device, exp, gyre_type, model_type, flow_type):
    ...
    dydt = dydt.reshape(1000, 1000, 2)
    ax_vecfield.scatter(true_y1.cpu().numpy()[0, :, 0], true_y1.cpu().numpy()[0, :, 1], c='blue', marker = 'o')
    ax_vecfield.scatter(pred_y1.cpu().numpy()[0, :, 0], pred_y1.cpu().numpy()[0, :, 1], c ='rebeccapurple', marker = 'o')
    ax_vecfield.scatter(true_y2.cpu().numpy()[0, :, 0], true_y2.cpu().numpy()[0, :, 1], c='green', marker = 'o')
    ax_vecfield.scatter(pred_y2.cpu().numpy()[0, :, 0], pred_y2.cpu().numpy()[0, :, 1], c ='yellow', marker = 'o')
    ax_vecfield.plot(true_y1.cpu().numpy()[:, :, 0], true_y1.cpu().numpy()[:, :, 1], 'b')
    ax_vecfield.plot(pred_y1.cpu().numpy()[:, :, 0], pred_y1.cpu().numpy()[:, :, 1], color ='rebeccapurple', linestyle='dashed')
    ax_vecfield.plot(true_y2.cpu().numpy()[:, :, 0], true_y2.cpu().numpy()[:, :, 1], 'g')
    ax_vecfield.plot(pred_y2.cpu().numpy()[:, :, 0], pred_y2.cpu().numpy()[:, :, 1], color ='yellow', linestyle='dashed')
    ax_vecfield.streamplot(x, y, dydt[:, :, 0], dydt[:, :, 1], color="black")
    if exp == "test":
        ax_vecfield.set_title('Learned Vector Field')
    else:
...
def visualize_single_gyre_streamplot(itr, t1,true_y1, pred_y1, odefunc, fig, ax_traj, ax_vecfield, device, exp, gyre_type, model_type, flow_type):
    ...
    if exp == 'test':
        pass
        # ax_traj.cla()
        # ax_traj.plot(t1.cpu().numpy(), true_y1.cpu().numpy()[:, :, 0], 'b', label='true x')
        # ax_traj.plot(t1.cpu().numpy(), true_y1.cpu().numpy()[:, :, 1], 'g', label='true y')
        # ax_traj.plot(t1.cpu().numpy(), pred_y1.cpu().numpy()[:, 0, 0],  color ='rebeccapurple', linestyle='dashed', label='pred x')
        # ax_traj.plot(t1.cpu().numpy(), pred_y1.cpu().numpy()[:, 0, 1], color ='yellow', linestyle='dashed', label='pred y')
        # ax_traj.plot(t1.cpu().numpy(), pred_y1.cpu().numpy()[:, 1, 0],  color ='rebeccapurple', linestyle='dashed', label='pred x')
        # ax_traj.plot(t1.cpu().numpy(), pred_y1.cpu().numpy()[:, 1, 1], color ='yellow', linestyle='dashed', label='pred y')
        # ax_traj.set_xlim(t1.cpu().min(), t1.cpu().max())
        # ax_traj.set_ylim(-100, 100)
        # ax_traj.set_title('Trajectories (left gyre)')
        # ax_traj.set_xlabel('t1')
    ...
    if flow_type == 'Var_':
        ...
        dydt = odefunc(0, torch.Tensor(np.stack([x, y], -1).reshape(1000 **2, 2)).to(device)).cpu().detach().numpy()
    dydt = dydt.reshape(1000, 1000, 2)
    ax_vecfield.scatter(true_y1.cpu().numpy()[0, :, 0], true_y1.cpu().numpy()[0, :, 1], c='orange', marker = 'o')
    ax_vecfield.scatter(pred_y1.cpu().numpy()[0, :, 0], pred_y1.cpu().numpy()[0, :, 1], c ='red', marker = 'o')
    ax_vecfield.plot(true_y1.cpu().numpy()[:, :, 0], true_y1.cpu().numpy()[:, :, 1], color='orange', label='ground_truth')
    ax_vecfield.plot(pred_y1.cpu().numpy()[:, :, 0], pred_y1.cpu().numpy()[:, :, 1], color ='red', linestyle='dashed', label='predicted')
    # ax_vecfield.plot(true_y1.cpu().numpy()[:, 1, 0], true_y1.cpu().numpy()[:, 1, 1], color='orange', label='ground_truth')
    # ax_vecfield.plot(pred_y1.cpu().numpy()[:, 1, 0], pred_y1.cpu().numpy()[:, 1, 1], color ='red', linestyle='dashed', label='predicted')
    # ax_vecfield.plot(true_y2.cpu().numpy()[:, 0, 0], true_y2.cpu().numpy()[:, 0, 1], 'g', label='true2')
...
def visualize_true_single_gyre(t1, true_y1, device, exp, model_type, flow_type):
    ...
    if exp == 'test':
        ...
        # ax_traj.plot(t1.cpu().numpy(), true_y1.cpu().numpy()[:, 0, 0], 'b', label='true x')
        # ax_traj.plot(t1.cpu().numpy(), true_y1.cpu().numpy()[:, 0, 1], 'g', label='true y')
        # ax_traj.plot(t1.cpu().numpy(), true_y1.cpu().numpy()[:, 1, 0], 'b', label='true x')
        # ax_traj.plot(t1.cpu().numpy(), true_y1.cpu().numpy()[:, 1, 1], 'g', label='true y')
        # ax_traj.set_xlim(t1.cpu().min(), t1.cpu().max())
        # ax_traj.set_ylim(-60, 60)
        # ax_traj.set_title('Trajectories (left gyre)')
        # ax_traj.set_xlabel('t')
    ...
    dydt = dydt.reshape(1000, 1000, 2)
    ax_vecfield.scatter(true_y1.cpu().numpy()[0, :, 0], true_y1.cpu().numpy()[0, :, 1], c = 'orange', marker='o')
    ax_vecfield.plot(true_y1.cpu().numpy()[:, :, 0], true_y1.cpu().numpy()[:, :, 1], 'orange', label='ground_truth')
    # ax_vecfield.plot(true_y1.cpu().numpy()[:, :, 0], true_y1.cpu().numpy()[:, :, 1], 'red', label='ground_truth')
    ax_vecfield.streamplot(x, y, dydt[:, :, 0], dydt[:, :, 1], color="black")
    ax_vecfield.set_xlim(-50,0)
    ax_vecfield.set_ylim(0, 50)
    ax_vecfield.set_title('True Vector Field')
...
def visualize_err_vecfield_knode(itr, true,func, fig_q, ax_true_vecfield, ax_pred_vecfield ,ax_err_vecfield, cbar_ax_1, cbar_ax_2, cbar_ax_3, device, exp, gyre_type, model_type, flow_type):
    ...
    if exp == "test":
        ax_pred_vecfield.set_title('NN vector field')
    else:
